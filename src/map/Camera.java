...
public class Camera {
	...
	public Camera(int posX, int posY,int width, int height, Map m) throws Exception {
	...
	public void render(Graphics g) {
		...
		for (int i = 0; i < 2; i++) {
			for (int j = 0; j < this.tileMap.get(i).get(j).size; j++) {
				if (this.tileMap.get(i).get(j).getX() < (positionX + pxwidth)
						&& this.tileMap.get(i).get(j).getY() < (positionY + pxheight)) {
					this.tileMap.get(i).get(j).render(g);
				}
	...
	public void setupTestGUI() {
		...
		frame.setSize(this.pxwidth, this.pxheight);// sets the size of the frame
